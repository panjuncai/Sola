...
**动作**
**动作**
- [x] 提供 `WebAudioProvider` 类（直接依赖 tRPC client 或 fetch）
- [x] Hook 中只注入 provider 实例，不再传闭包函数
- [ ] 可替换 provider（Mock / LRU Cache / Offline）

**验收**
- [x] Scheduler 可以脱离 React 运行
- [ ] provider 可独立测试/替换

---

## 里程碑验收清单（v3）
- [x] PlaybackStrategy 含 `nextIndex` + `getRoleOrder`
- [x] Hook 层不再定义 `loop-target` 规则
- [x] Scheduler 实例稳定（useRef/useState lazy init）
- [x] 播放中配置变化不会中断
