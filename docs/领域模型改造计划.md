...
### 动作
- [x] 新建 `packages/logic/src/playback/engine.ts`
- [x] 设计 `PlaybackEngine`（类 or 状态机）
...
- Engine 独立单测覆盖核心播放逻辑
- Hook 代码量显著下降
  - 进度：已接入 `useArticleToolbarLogic` 的 shadowing/循环播放与角色顺序构建
  - 进度：已抽出 `useTtsAudioProvider`（TTS 缓存/请求/播放）作为 Data Provider
  - 进度：已抽出 `AudioPlayer` 纯类（播放逻辑与 React 解耦）
...
### 动作
- [x] 新建 `SentenceEntity`（`packages/logic/src/article/entities.ts`）
- [x] 新建 `ArticleEntity`
- [x] 将 “可挖空/主展示角色”逻辑迁入 `SentenceEntity`
- [x] 将 “是否可播放”等其余逻辑迁入实体
- [x] 关键 UI 改为调用实体方法或 getter（`SentenceItem` / `SentenceRoleRow` / sidebar title）
...
### 动作
- [x] 统一 DTO 放入 `packages/shared`（ArticleDetailResponse / TtsOptionsResponse / TtsOptionsInput）
- [x] `logic` / `web` 使用 shared 类型
- [x] `api` 使用 shared 类型（article list/get + tts options output）
- [x] 删除重复类型定义
...
- [x] logic 目录已建立 + 边界规则通过
- [x] Article 解析逻辑已下沉并复用
- [x] PlaybackEngine 可测试、UI Hook 变薄
- [x] 领域实体类承载核心判断逻辑
- [x] shared 统一类型语言落地
